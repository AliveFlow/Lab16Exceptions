...
int main()
{
	...
	while (true) {
		...
		if (ans == 1) {
			...
			loadPredmet("subjects.txt", &predmets_list);
		}
		else if (ans == 2) {
			
			loadStudent("students.txt", &peoples_list, &predmets_list);
		}
		else if (ans == 3) {
			...
			getline(cin, name);
			cout << endl;
			predmets_list.push_back(new Predmet(name, predmets_list.size()+1));
		}
		else if (ans == 6) {
			cout << "Введите имя студента - ";
			...
			do {
				...
				try {
					...
					if ((cin >> mark)&&(mark>=1 && mark <=5)&&(floor(mark) == mark))
					{
					...
					{
						throw "kek";
						cout << "Ошибка. Введите оценку от 1 до 5" << endl;
